...
//				String sProjectID, String sProjectChangeDate, String sProjectBeginDate, String sProjectEndDate)
	...
	public void testGetFeatureEstimates_ManySameSuperFeatures_OneSuperFeatureRs() {
		when(componentRepository.findById(mockComponentId)).thenReturn(Optional.ofNullable(mockComponent));
		when(collectorRepository.findById(mockItem2.getCollectorId())).thenReturn(Optional.ofNullable(mockJiraCollector));
	...
	public void testGetCurrentSprintDetail_ValidKanbanTeam_ShowKanban() {
		when(componentRepository.findById(mockComponentId)).thenReturn(Optional.ofNullable(mockComponent));
		when(collectorRepository.findById(mockItem3.getCollectorId())).thenReturn(Optional.ofNullable(mockJiraCollector));
